...
> **Publish gate (v0.1):** do **not** publish to npm until the full live WebMCP acceptance
> matrix **rows A–H** passes in a WebMCP-enabled Chrome runtime (see
> `docs/acceptance-real-browser.md`). Row I (the no-runtime path) is already verified. Until
> A–H are green, the package is prepared and extractable but unpublished.
