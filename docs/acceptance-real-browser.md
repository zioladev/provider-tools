...
>
> **Publish gate:** `@zioladev/provider-tools` is **not** published to npm until **rows A–H
> pass** in a WebMCP-enabled Chrome. Row I (no-runtime) is already verified. See
...
Produce a short results doc: the Chrome version + active flag/surface, the row A–I
pass/fail table, and the §4 Match/Differs list with any proposed one-line kit fixes. If
§4.1 (arg delivery) or §4.2 (schema form) differs, open an issue and add a
regression-style unit test that encodes the real runtime's contract before changing the
built-tool wrapper.
