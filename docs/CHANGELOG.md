...
### Verified
- Live WebMCP acceptance (Chrome Canary, `document.modelContext`): registration, live tool
  surface, read + state-changing execution, input rejection, and structured evidence all
  pass; the runtime delivers handler args as objects, so no kit changes were required. See
  `docs/acceptance-real-browser.md` §6.
